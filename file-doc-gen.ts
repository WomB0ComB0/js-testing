...
class JSDocGenerator {
   ...
   */
  private async makeApiCall(fileContent: string, attempt: number = 1): Promise<string> {
    try {
   ...
   * ```
   */
  async processFile(inputPath: string, outputPath?: string, overwrite = false): Promise<ProcessingResult> {
  ...
  async processFiles(filePaths: string[], overwrite = false): Promise<ProcessingResult[]> {
    const startTime = Date.now();
    console.log(`🚀 Starting processing of ${filePaths.length} files with max concurrency: ${this.config.maxConcurrency}`);
   ...
   * ```
   */
  async scanGitFiles(directoryPath = '.', includeUntracked = true, additionalExcludePatterns: string[] = []): Promise<string[]> {
    ...
    try {
      ...
      const filteredFiles = additionalExcludePatterns.length > 0 
      ...
      console.log(`📁 Found ${filteredFiles.length} supported files using Git${includeUntracked ? ' (including untracked)' : ''}`);
      
      return filteredFiles;
    } catch (error) {
      // Fallback to directory scanning if not in a Git repo
      console.warn('⚠️  Git not available or not in a Git repository, falling back to directory scanning');
   ...
   * ```
   */
  async scanDirectory(directoryPath: string, recursive = false, excludePatterns: string[] = []): Promise<string[]> {
    const supportedExtensions = ['.ts', '.js', '.tsx', '.jsx'];
    const defaultExcludePatterns = ['node_modules', '.git', 'dist', 'build', 'coverage', '*.test.*', '*.spec.*'];
    ...
    console.log(`📁 Found ${files.length} supported files in ${directoryPath}${recursive ? ' (recursive)' : ''}`);
  ...
  updateConfig(newConfig: Partial<GenerationConfig>): void {
    if (newConfig.maxConcurrency && newConfig.maxConcurrency !== this.config.maxConcurrency) {
      throw new Error('Cannot update maxConcurrency after initialization. Create a new instance instead.');
    }
...
async function main() {
  ...
  if (concurrencyIndex !== -1 && concurrencyIndex < args.length - 1) {
    maxConcurrency = parseInt(args[concurrencyIndex + 1], 10);
  ...
  if (retryIndex !== -1 && retryIndex < args.length - 1) {
    retryAttempts = parseInt(args[retryIndex + 1], 10);
  ...
  const flagsToSkip = new Set(['--overwrite', '--no-git', '--no-untracked', '--concurrency', '--retry-attempts', '--dir', '--exclude']);
  const files = [];
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (flagsToSkip.has(arg)) {
      // Skip the flag
      if (arg === '--concurrency' || arg === '--retry-attempts' || arg === '--dir' || arg === '--exclude') {
  ...
  try {
    ...
    if (targetDirectory) {
      // Process directory
      results = await generator.processDirectory(targetDirectory, useGit, includeUntracked, overwrite, excludePatterns);
    } else if (files.length === 0) {
      // Default to current directory if no files specified
      results = await generator.processDirectory('.', useGit, includeUntracked, overwrite, excludePatterns);
    } else {
      // Process specific files
      const supportedExtensions = ['.ts', '.js', '.tsx', '.jsx'];
      const validFiles = [];
