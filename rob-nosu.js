...
class GameStats {
    ...
    updateMoneyStats(currentMoney) {
        ...
        if (timeDiff > 0) {
            this.moneyStats.perSecond = (currentMoney - this.moneyStats.previousCheck) / timeDiff;
...
class UpgradeManager {
    ...
    calculateEfficiencyScore(upgrade) {
        ...
        const diminishingFactor = Math.pow(0.95, upgrade.owned);
        const tierBonus = Math.log10(upgrade.cost) / 10;
        const rateOfReturnDays = upgrade.cost / (upgrade.revenuePerSecond * 86400);

        return baseEfficiency * diminishingFactor * (1 + tierBonus) * (1 / (1 + rateOfReturnDays));
    }

    async findAndPurchaseBestUpgrade(currentMoney) {
        const upgrades = Array.from(document.querySelectorAll(CONFIG.selectors.upgrades))
    ...
    parseUpgradeInfo(element) {
        try {
            const getText = selector => element.querySelector(selector)?.textContent || '';
            ...
            return Object.values(info).every(val => val !== undefined) ? info : null;
        } catch (error) {
    ...
    async purchaseUpgrade(upgrade) {
        ...
        try {
            ...
            // Update history
            const history = this.stats.upgradeHistory.get(upgrade.name) || { purchases: 0, totalSpent: 0 };
            history.purchases++;
            history.totalSpent += upgrade.cost;

            this.stats.upgradeHistory.set(upgrade.name, history);
            console.log(`Purchased upgrade: ${upgrade.name}, Spent: $${CONFIG.logging.dateFormat.format(upgrade.cost)},  Owned: ${upgrade.owned + 1}`);
            return true;
...
class AccompliceRecruiter {
    ...
    async attemptRecruitment() {
        ...
        try {
            if (this.state === CONFIG.accomplice.states.COOLDOWN &&
    ...
    async openDialog() {
        try {
            ...
            recruitButton.click();
            await new Promise(resolve => setTimeout(resolve, CONFIG.accomplice.timings.dialogWait));
            return true;
        } catch (error) {
    ...
    async enterEmail() {
        try {
            ...
            await new Promise(resolve => setTimeout(resolve, CONFIG.accomplice.timings.emailWait));
            return true;
    ...
    async submitInvitation() {
        try {
            ...
            sendButton.click();
            await new Promise(resolve => setTimeout(resolve, CONFIG.accomplice.timings.submitWait));
            return true;
    ...
    findRecruitButton() {
        ...
        return buttons.find(button => button.textContent.includes(CONFIG.accomplice.recruitButtonText));
    }

    findSendButton(modal) {
        if (!modal) return null;
        const buttons = Array.from(modal.querySelectorAll('button'));
        return buttons.find(button => button.textContent.includes(CONFIG.accomplice.sendButtonText));
    }
...
class GameAutomation {
    ...
    scheduleNextRun() {
        this.automationInterval = setTimeout(() => this.automateGame(), CONFIG.timing.checkInterval);
    }
    ...
    async start() {
        ...
        try {
            const gameContainer = document.querySelector(CONFIG.selectors.gameContainer);
            if (!gameContainer) {
