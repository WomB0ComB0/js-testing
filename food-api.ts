...
// FoodData Central API Client - Fixed and improved implementation

/**
 * Food data types based on FDC API structure
 */
 ...
 */
export type DataType = 'Branded' | 'Foundation' | 'Survey' | 'Legacy' | 'Survey (FNDDS)' | 'SR Legacy';
...
export class FDCRateLimitError extends FDCApiError {
  constructor(message = 'Rate limit exceeded. API key temporarily blocked for 1 hour.') {
    super(message, 429);
...
export class FDCClient {
  ...
  private async makeRequest<T>(
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        if (error instanceof FDCRateLimitError || error instanceof FDCAuthError) {
  ...
  private async handleHttpError(response: Response): Promise<never> {
    ...
    switch (response.status) {
      ...
      case 404:
        throw new FDCApiError(`Resource not found. Check the endpoint URL and FDC ID.`, 404, responseText);
      case 400:
        throw new FDCApiError(`Bad request: ${responseText}`, 400, responseText);
      case 500:
        throw new FDCApiError(`Server error: ${responseText}`, 500, responseText);
      default:
   ...
   */
  async getFoodsList(options: ListOptions = {}): Promise<FoodsListResponse | null> {
    try {
      const params: Record<string, any> = {
      ...
      if (error instanceof FDCApiError && error.status === 404) {
        console.warn('getFoodsList: /foods/list endpoint not available. Use searchFoods instead.');
        return null;
      }
      throw error;
    }
   ...
   */
  async getFoodNutrients(fdcId: number, nutrientIds?: number[]): Promise<FoodNutrient[]> {
   ...
   */
  async findFood(query: string, dataType?: DataType[]): Promise<AbridgedFood | null> {
   ...
   */
  async getFoodsListAlternative(options: ListOptions = {}): Promise<SearchResult> {
 ...
 */
export function createFDCClient(apiKey: string, config?: Partial<FDCClientConfig>): FDCClient {
...
async function testFDCClient() {
  ...
  try {
    ...
    if (foodsList) {
      ...
      console.log('Alternative Foods List:', alternativeList.foods.length, 'foods');
    }
