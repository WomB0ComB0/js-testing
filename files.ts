...
const convertCase = (
	...
	if (!converter) {
		console.error(
			`Error: Unsupported case type "${targetCase}". Supported cases are: ${Object.keys(caseConverters).join(", ")}`,
		);
		return;
	}
...
(() => {
	try {
		...
		if (!inputDir || !targetCase) {
			console.log(
				"Usage: node your-script.js <directoryPath> <targetCase> [fileExtensions...]",
			);
		...
		if (extensions.length > 0) {
			console.log(
				`Only processing files with extensions: ${extensions.join(", ")}`,
			);
		} else {
