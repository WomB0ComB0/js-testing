...
class GoogleMapsService {
  ...
  private readonly directionsBaseUrl = 'https://maps.googleapis.com/maps/api/directions/json';
  private readonly placesAutocompleteBaseUrl = 'https://maps.googleapis.com/maps/api/place/autocomplete/json';
  private readonly geolocationBaseUrl = 'https://www.googleapis.com/geolocation/v1/geolocate';
  private readonly streetViewBaseUrl = 'https://maps.googleapis.com/maps/api/streetview';
  private readonly streetViewMetadataBaseUrl = 'https://maps.googleapis.com/maps/api/streetview/metadata';
  private readonly elevationBaseUrl = 'https://maps.googleapis.com/maps/api/elevation/json';
  private readonly timeZoneBaseUrl = 'https://maps.googleapis.com/maps/api/timezone/json';
  ...
  async getDistanceMatrix(
    ...
    try {
      ...
      const originsStr = originsArray.map(origin => {
        if (typeof origin === 'string') return origin;
        return `${origin.lat},${origin.lng}`;
      }).join('|');
      
      const destinationsStr = destinationsArray.map(destination => {
      ...
      if (response.data.status !== 'OK') {
        console.error(`Distance Matrix API failed with status: ${response.data.status}`);
        return null;
      }
      ...
      throw new Error(`Failed to get distance matrix: ${(error as Error).message}`);
    }
  }
  ...
  async getDirections(
    ...
    try {
      ...
      if (options.waypoints && options.waypoints.length > 0) {
        waypointsStr = options.waypoints.map(wp => {
      ...
      if (response.data.status !== 'OK') {
        console.error(`Directions API failed with status: ${response.data.status}`);
        return null;
      }
  ...
  getMapsJavaScriptTag(options: {
    ...
    const libraries = options.libraries ? `&libraries=${options.libraries.join(',')}` : '';
  ...
  async getPlacesAutocomplete(
    ...
    try {
      ...
      if (response.data.status !== 'OK') {
        console.error(`Places Autocomplete API failed with status: ${response.data.status}`);
        return null;
      }
      ...
      throw new Error(`Failed to get autocomplete predictions: ${(error as Error).message}`);
    }
  }
  ...
  async getCurrentLocation(
    ...
    try {
      ...
      const response = await axios.post(`${this.geolocationBaseUrl}?key=${this.apiKey}`, payload);
  ...
  getStreetViewImageUrl(
    ...
    if (options.heading !== undefined) params.append('heading', options.heading.toString());
    if (options.pitch !== undefined) params.append('pitch', options.pitch.toString());
    ...
    if (options.return_error_code !== undefined) params.append('return_error_code', options.return_error_code.toString());
  ...
  async getStreetViewMetadata(
    ...
    try {
      ...
      // One of location or pano is required
      if (!options.location && !options.pano) {
        throw new Error('Either location or pano is required for Street View metadata');
      }
      ...
      const response = await axios.get(this.streetViewMetadataBaseUrl, { params });

      return response.data;
    } catch (error) {
      console.error('Street View Metadata Error:', error);
      throw new Error(`Failed to get Street View metadata: ${(error as Error).message}`);
    }
  }
  ...
  async getElevation(
    ...
    try {
      let params: Record<string, string> = { key: this.apiKey };
      ...
      if (response.data.status !== 'OK') {
        console.error(`Elevation API failed with status: ${response.data.status}`);
        return null;
      }
  ...
  async getTimeZone(
    ...
    try {
      ...
      if (response.data.status !== 'OK') {
        console.error(`Time Zone API failed with status: ${response.data.status}`);
        return null;
      }
      ...
      throw new Error(`Failed to get time zone data: ${(error as Error).message}`);
    }
  }
...
async function main() {
  ...
  try {
    ...
    await mapsService.testDirections(
      'Mountain View, CA', 
      'San Francisco, CA'
    );
    ...
    await mapsService.testStreetView('1600 Amphitheatre Parkway, Mountain View, CA');
